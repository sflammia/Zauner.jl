...
\\
Compute one element from each coset of `M/S` where `M` is a maximal abelian subgroup of `GL(2,ℤ/d')` and `S` is the stabilizer of `F`. 
...
\\
Compute the orders of the elements of the Galois group and a maximal p orbit.
"""
